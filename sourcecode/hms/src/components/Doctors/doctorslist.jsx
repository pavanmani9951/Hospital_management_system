...
class DoctorsList extends Component {
  ...
  componentDidMount() {
    this.onSetTotalNumODoctors();
    this.onFetchData(this.state.limit);
  }
  async onSetTotalNumODoctors() {
    this.setState({ isLoading: true });
    const res = await Service.getTotalNumOfPerson("doctors");
    if (res === "error") {
      console.log("error");
      this.setState({ isLoading: false });
    } else {
      this.setState({ totalNumOfDoctors: res, isLoading: false });
    }
  }
  ...
  showMore() {
    if (this.state.limit <= this.state.totalNumOfDoctors) {
      ...
      console.log(this.state.totalNumOfDoctors);
  ...
  handleSeach = async () => {
    if (this.state.serachText === "" || null) {
      ...
      if (resultPatientlist === "error") {
        ...
        this.setState({
          doctorsList: resultPatientlist,
          isSearching: false,
        });
      }
    }
  };
  // onSetTotalNumODoctors() {
  //   const db = firebase.firestore();
  //   db.collection("doctors")
  //     .get()
  //     .then((snapshot) => {
  //       const doctorsList = [];
  //       this.setState({ totalNumOfDoctors: snapshot.docs.length });
  //     })
  //     .catch((e) => {
  //       console.log(e);
  //     });
  ...
  async onFetchData(limit) {
    ...
    const fetchedDataList = await Service.getData("doctors", limit);

    if (fetchedDataList.length !== 0) {
      this.setState({
        doctorsList: fetchedDataList,
        isLoadMoredata: false,
        isLoading: false,
      });
    } else {
      ...
      <div className="doctorsListpage">
        <div className="main_section">
          ...
          <AlertDialogBox
            ...
            title="Added"
            des="New doctor has been added sccessfully"
          ></AlertDialogBox>
          <ErorrDialogBox
          ...
          <div className="loadmoredatasection">
            ...
            {this.state.doctorsList.length === 0 ? null : this.state
...
export default connect(null, mapDisptachToProps)(DoctorsList);
